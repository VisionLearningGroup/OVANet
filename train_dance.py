...
from utils.loss import entropy_margin, entropy
...
params = []
lemniscate = LinearAverage(dim, ndata, conf.model.temp, conf.train.momentum).cuda()
...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        loss_nc = conf.train.eta * entropy(torch.cat([feat_mat,
                                                      feat_mat2, out_t], 1))
        loss_nt = entropy_margin(out_t, conf.train.thr,
                                                  conf.train.margin)
        all = loss_s + args.multi * (loss_nt + loss_nc) * 0.5
