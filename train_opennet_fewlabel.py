...
from utils.defaults import get_dataloaders_label
...
parser.add_argument('--target_label', type=str, default='./utils/target_list.txt', metavar='B',
                    help='path to target list')
...
parser.add_argument('--neptune', dest='neptune',
                    ...
                    action='store_true')
parser.add_argument('--closed', dest='closed', default=False,
                    help='whether we do closed set evaluation',
                    action='store_true')
...
target_data_label = args.target_label
evaluation_data = args.target_path
network = args.net
source_loader, target_loader, target_loader_label, \
test_loader, target_folder = get_dataloaders_label(source_data,
                                             target_data,
                                             target_data_label,
                                             evaluation_data,
                                             conf)
...
open = n_total-n_share-n_source_private > 0 and not args.closed
...
def train():
    ...
    data_iter_t_l = iter(target_loader_label)
    len_train_source = len(source_loader)
    len_train_target = len(target_loader)
    len_train_target_l = len(target_loader_label)
    weight = torch.ones(2).float().cuda()
    weight[0] = num_class-1#1. / (num_class-1)
    weight = 1.0 / weight
    ...
    for step in range(conf.train.min_step + 1):
        ...
        feat_t_l = G(img_t_l)
        out_t_l = C1(feat_t_l)
        out_open_l = C2(feat_t_l)
        out_open_l = out_open_l.view(out_t_l.size(0), 2, -1)
        label_t_sp = torch.zeros((out_open_l.size(0),
                                  out_open_l.size(2))).long().cuda()
        label_range_l = torch.range(0, out_open_l.size(0) - 1).long()
        label_t_sp[label_range_l, label_t_l] = 1
        loss_t_l = criterion(out_t_l, label_t_l)
        loss_t_open = criterion_open(out_open_l, label_t_sp)
        ...
        out_t = F.softmax(C1(feat_t))
        out_open_t = out_open_t.view(img_t.size(0), 2, -1)
        out_open_t = F.softmax(out_open_t, 1)
        pred = out_t.data.max(1)[1]
        pred_v = out_t.data.max(1)[0]
        tmp_range = torch.range(0, out_t.size(0) - 1).long().cuda()
        pred_knw = out_open_t[tmp_range, 1, pred]
        ent_open = torch.mean(torch.mean(torch.sum(-out_open_t *
                                                   torch.log(out_open_t + 1e-8), 1), 1))
        weight = 1 - (1 + 10 * min(1.0, step / conf.train.min_step)) ** (-0.75)
        #conf_share = torch.mean((pred_v-pred_knw)**2)
        all = loss_s + loss_st + ent_open * weight + loss_t_l + loss_t_open
        ...
        if step % conf.train.log_interval == 0:
            print('Train [{}/{} ({:.2f}%)]\tLoss Source: {:.6f} '
                  'Loss Open: {:.6f} Loss Target: {:.6f} Loss Target Open: {:.6f} Unl Target: {:.6f}, Weight {:.6f}\t'.format(
                step, conf.train.min_step,
                100 * float(step / conf.train.min_step),
                loss_s.item(), loss_st.item(),loss_t_l.item(), loss_t_open.item(),  ent_open.item(), weight))
