...
test_loader, target_folder = get_dataloaders(source_data=source_data,
...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        ##import pdb
        #pdb.set_trace()
        index_neg = torch.multinomial(label_sp_neg.float(), num_samples=1)
        label_s_neg = torch.zeros((out_open.size(0),
                                  out_open.size(2))).long().cuda()
        label_s_neg[label_range, index_neg.view(-1)] = 1
        #torch.multinomial(label_sp_neg)
        open_loss = torch.mean(torch.sum(-torch.log(out_open[:, 1, :]+1e-8) * label_s_sp, 1))
        open_loss_neg = torch.mean(torch.max(-torch.log(out_open[:, 0, :]+1e-8) * label_sp_neg, 1)[0])
        #import pdb
        #pdb.set_trace()
        ## b x 2 x C
        loss_st = 0.5 * (open_loss + open_loss_neg)
        ...
        if step % conf.train.log_interval == 0:
            print('Train [{}/{} ({:.2f}%)]\tLoss Source: {:.6f} '
                  'Loss Open: {:.6f} Loss Open Target: {:.6f} Loss Open Source Positive: {:.6f}\t Loss Open Source Negative: {:.6f}\t'.format(
                step, conf.train.min_step,
                100 * float(step / conf.train.min_step),
                loss_s.item(), loss_st.item(), ent_open.item(), open_loss.item(), open_loss_neg.item()))
